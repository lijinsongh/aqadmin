<template>
	<el-tabs v-model="activeName" @tab-click="handleClick">
		...
		<el-tab-pane label="首页导航" name="second">
			<div style="float: right;margin-right:2%;">
				<el-button style="margin: 10px 0;" :disabled="!isAuth('storeList:add')" size="mini" type="primary"
					icon="document" @click="compileSy()">添加导航</el-button>
			</div>
			<el-table v-loading="tableDataLoading" :data="homeDataSy">
				<el-table-column fixed prop="id" label="编号" width="80">
				</el-table-column>
				<el-table-column prop="title" label="名称">
				</el-table-column>
				<el-table-column prop="content" label="副标题">
				</el-table-column>
				...
				<el-table-column prop="state" label="类型">
					<template slot-scope="scope">
						<span v-if="scope.row.state==1">金刚区</span>
						<span v-if="scope.row.state==2">精选好物</span>
						<span v-if="scope.row.state==3">热卖榜单</span>
						<span v-if="scope.row.state==4">每日上新</span>
						<span v-if="scope.row.state==5">拼团购</span>
						<span v-if="scope.row.state==6">邀请图</span>
						<span v-if="scope.row.state==7">首页邀请图</span>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" width="180">
					<template slot-scope="scope">
						<el-button size="mini" type="primary" :disabled="!isAuth('storeList:update')"
							@click="compileSy(scope.row)">编辑
						</el-button>
						<el-button size="mini" type="danger" :disabled="!isAuth('storeList:delete')"
							@click="compiledeletes(scope.row)">
							删除
						</el-button>
					</template>
				</el-table-column>
			...
			<el-dialog :title="titles" :visible.sync="dialogFormVisible10" center>
				<el-form :model="form1">
					...
					<el-form-item label="副标题：" :label-width="formLabelWidth">
						<el-input v-model="form1.content" style="width:65%;"></el-input>
					</el-form-item>
					<el-form-item label="路由：" :label-width="formLabelWidth">
						<el-input v-model="form1.url" style="width:65%;"></el-input>
					</el-form-item>
					<!-- <el-form-item label="类型：" :label-width="formLabelWidth" v-if="titles=='修改'">
					...
					<el-form-item label="图片：" :label-width="formLabelWidth">
						<div
							...
							<el-upload class="avatar-uploader" v-model="image_url"
								action="https://sd.shengqianxiong.com.cn/tao/alioss/upload" :show-file-list="false"
				...
				<div slot="footer" class="dialog-footer">
					...
					<el-button type="primary" @click="compileNoticeToSy()">确 定</el-button>
				</div>
			</el-dialog>
		</el-tab-pane>
          ...
          <el-option v-for="item in platforms" :key="item.value" :label="item.label" :value="item.value">
			...
			<el-table v-loading="tableDataLoading" :data="homeData.content">
				...
				<el-table-column prop="type " label="状态">
					<template slot-scope="scope">
						<el-switch v-model="scope.row.type" @change="change(scope.row.type,scope.row)"
	...
	export default {
		...
		methods: {
			...
			releaslunNoticeTo() {
				...
				this.$http({
					url: this.$http.adornUrl2(
						`/banner/update/-1?url=${this.url}&image=${this.image_url}&type=${this.type}`),
					method: 'post',
					data: this.$http.adornData({})
				}).then(({
					data
			...
			homeSelectSy() {
				...
				this.$http({
					url: this.$http.adornUrl2(
						`/selfActivity/list?page=${this.page-1}&size=${this.size}&type=${this.types1}`),
					method: 'get',
					params: this.$http.adornParams({})
				}).then(({
					data
			...
			compileSy(rows) {
				...
				if (rows) {
					...
					this.form1.content = rows.content
			...
			compiledeletes(row) {
				this.$confirm(`确定删除此条信息?`, '提示', {
					...
					this.$http({
						url: this.$http.adornUrl2(`/selfActivity/delete?id=${row.id}`),
						method: 'get',
						data: this.$http.adornData({})
					}).then(({
						data
