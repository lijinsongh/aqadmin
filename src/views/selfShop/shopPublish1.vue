<template>
	<div class="components-container main-cont">
			...
			<div style="position: relative;">
      		...
      		<el-input style="width:200px;" class="margin15" placeholder="请输入商户号"  v-model="merchants" autosize></el-input>
				...
				<el-row>
					<!-- <el-col :span="6">
						<span>生产日期：</span>
						<el-date-picker class="margin15" style="width:200px;" format="yyyy-MM-dd"
							value-format="yyyy-MM-dd" v-model="productionDate" type="date" placeholder="选择生产日期">
						</el-date-picker>
					</el-col>
					<el-col :span="6">
						<span>到期时间：</span>
						<el-date-picker class="margin15" style="width:200px;" format="yyyy-MM-dd"
							value-format="yyyy-MM-dd" v-model="expirationDate" type="date" placeholder="选择到期时间">
						</el-date-picker>
					</el-col> -->
					<el-col :span="6">
						<span>必买理由：</span>
						<el-input style="width:200px;" class="margin15" placeholder="请输入必买理由" v-model="buyReason"
							autosize></el-input>
					</el-col>
        ...
        <div style="display: flex;align-items: center;margin: 2% 0;flex-flow: wrap;">
          <span style="display: inline-block;text-align: right;">商品轮播图：</span>
          <div>
            <el-upload action="https://sanshengadmin.hnqxkjfz.com/tao/alioss/upload" list-type="picture-card"
              ...
              <i class="el-icon-plus"></i>
                      ...
                      <el-upload style="width: 60px;height: 60px;" class="avatar-uploader" v-model="scope.row.skuImg"
                        ...
                        <i v-else class="el-icon-plus avatar-uploader-icon" @click="curRowIndex=scope.$index"></i>
							...
							<el-table-column prop="sales" width="120">
								<template slot-scope="scope">
									<div>
										<el-input type="text" v-model="scope.row.sales"
											style="width:80px;font-size:14px;">
										</el-input>
									</div>
								</template>
				...
				<div style="height: 38px;line-height: 38px;margin:20px 0;">
					...
					<el-radio-group v-model="positage" @change="agreeChange">
						...
						<el-input v-if="priceshow" style="width:200px;margin-left: 10px;" placeholder="请输入邮费"
							type="number" v-model="postagePrice" autosize></el-input>
					</el-radio-group>
				</div>
			</div>
			...
			<div class="tinymce-content" style="display: flex;position: relative;">
				<div style="width:90px;">商品详情：</div>
				<!-- <div style="cursor: pointer;height: 40px;width: 127px;position: absolute;left: 100px;">
					<el-button slot="trigger" size="small" type="primary" @click="btnMethod()">选取图片库图片</el-button>
				</div> -->
				<quill-editor ref="myTextEditor" v-model="descrition" :options="quillOption"
					style="padding-bottom: 50px;height: 300px;width: 72%;display: inline-table;margin-bottom: 60px;">
				</quill-editor>
		...
		<el-dialog title="选择图片上传" :visible.sync="centerDialogVisible" width="30%" center>
			<div style="text-align: center;">
				<el-radio v-model="imgType" :label="1">图片库上传</el-radio>
				<el-radio v-model="imgType" :label="2">本地上传</el-radio>
			</div>

			<span slot="footer" class="dialog-footer">
				<el-button @click="centerDialogVisible = false">取 消</el-button>
				<el-button type="primary" @click="imageBtn()" v-if="imgType==1&&imgIndex==1">确 定</el-button>
				<el-upload class="avatar-uploader" v-model="coverImg" v-if="imgType==2 &&imgIndex==1"
					action="https://tbshopsadmin.hnqxkjfz.com/tao/alioss/uploads" :show-file-list="false"
					:on-success="handleAvatarSuccess1" style="display: inline-block;margin-left: 10px;">
					<el-button type="primary" @click="centerDialogVisible = false">确 定</el-button>
				</el-upload>
				<el-button type="primary" @click="imageBtn()" v-if="imgType==1&&imgIndex==2">确 定</el-button>
				<el-upload class="avatar-uploader" v-model="coverImg" v-if="imgType==2 &&imgIndex==2"
					action="https://tbshopsadmin.hnqxkjfz.com/tao/alioss/uploads" :show-file-list="false"
					:on-success="handleUploadSuccess" style="display: inline-block;margin-left: 10px;">
					<el-button type="primary" @click="centerDialogVisible = false">确 定</el-button>
				</el-upload>

				<el-button type="primary" @click="imageBtn()" v-if="imgType==1&&imgIndex==5">确 定</el-button>
				<el-upload v-if="imgType==2 &&imgIndex==5" class="avatar-uploader" v-model="valueData2[0].skuImg"
					action="https://tbshopsadmin.hnqxkjfz.com/tao/alioss/uploads" :show-file-list="false"
					:on-success="handleAvatarSuccess3" style="display: inline-block;margin-left: 10px;">
					<el-button type="primary" @click="centerDialogVisible = false">确 定</el-button>
				</el-upload>

				<el-button type="primary" @click="imageBtn()" v-if="imgType==1&&imgIndex==6">确 定</el-button>
				<el-upload v-if="imgType==2 &&imgIndex==6" class="avatar-uploader"
					v-model="valueData[curRowIndex].skuImg"
					action="https://tbshopsadmin.hnqxkjfz.com/tao/alioss/uploads" :show-file-list="false"
					:on-success="handleAvatarSuccess" style="display: inline-block;margin-left: 10px;">
					<el-button type="primary" @click="centerDialogVisible = false">确 定</el-button>
				</el-upload>

				<!--
				<el-upload style="width: 60px;height: 60px;" class="avatar-uploader"
					v-model="scope.row.skuImg"
					action="https://tbshopsadmin.xianmaxiong.com/tao/alioss/uploads"
					:show-file-list="false" :on-success="handleAvatarSuccess">
					<img v-if="scope.row.skuImg" :src="scope.row.skuImg" class="avatar"
						style="border-radius: 6px;width:60px;height: 60px;" />
					<i v-else class="el-icon-plus avatar-uploader-icon"
						@click="curRowIndex=scope.$index"></i>
				</el-upload> -->


			</span>
		</el-dialog>
		<el-dialog title="图片库" :visible.sync="centerDialogVisible1" width="800px" center>
			<div style="margin-right:2%;display: inline-block;">
				<span>图片名称：</span>
				<el-input v-model="imageNames" @keydown.enter.native="select" placeholder="请输入图片名称"
					style="width:200px;">
				</el-input>&nbsp;&nbsp;&nbsp;
				<el-button icon="document" size="mini" style="margin: 10px 0;" type="primary" @click="select">查询
				</el-button>
				&nbsp;&nbsp;&nbsp;
				<el-button icon="document" size="mini" style="margin: 10px 0;" type="primary" @click="clear">重置
				</el-button>&nbsp;&nbsp;&nbsp;
				<el-button size="mini" type="primary" @click="querenBtnDuo()">确定
				</el-button>
			</div>
			<el-checkbox-group v-model="imageUrlList">
				<el-checkbox v-for="(item,index) in typeDatasImg.content" :key="index" :label="item.imageUrl"
					style="display: inline-block;margin: 2%;width: 16%;height: 200px;position: relative;" class="chesks">
					<img style="width: 100px; height: auto;border-radius: 10px;" :src="item.imageUrl" alt="">
					<!-- <el-popover placement="top-start" title="" trigger="hover" style="width: 100px;">
						<img style="width: 100px; height: auto;border-radius: 10px;" :src="item.imageUrl" alt=""
							slot="reference">
						<img style="width: 400px; height: auto" :src="item.imageUrl" alt="">
					</el-popover> -->
					<div style="text-align: center;margin-top: 10px;color: #666;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;-o-text-overflow:ellipsis;width: 100px;">{{item.imageName}}</div>
				</el-checkbox>
			</el-checkbox-group>

			<div style="text-align: center;margin-top: 10px;">
				<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
					:page-sizes="[10, 20, 30]" :page-size="size" :current-page="page"
					layout="total,sizes, prev, pager, next,jumper" :total="typeDatasImg.totalElements">
				</el-pagination>
			</div>
		</el-dialog>
		<el-dialog title="图片库" :visible.sync="centerDialogVisible2" width="800px" center>
			<div style="margin-right:2%;display: inline-block;">
				<span>图片名称：</span>
				<el-input v-model="imageNames" @keydown.enter.native="select" placeholder="请输入图片名称"
					style="width:200px;">
			...
			<div>
				<div v-for="(item,index) in typeDatasImg.content" :key="index"
					style="display: inline-block;margin: 2%;width: 16%;height: 200px;position: relative;">
					<img style="width: 100px; height: auto;border-radius: 10px;" :src="item.imageUrl" alt="" @click="querenBtn(item)">
					<!-- <el-popover placement="top-start" title="" trigger="hover" style="width: 100%;">
						<img style="width: 100%; height: auto;border-radius: 10px;" :src="item.imageUrl" alt=""
							slot="reference" @click="querenBtn(item)">
						<img style="width: 400px; height: auto" :src="item.imageUrl" alt="">
					</el-popover> -->
					<div style="width: 100px;text-align: center;margin-top: 10px;color: #666;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;-o-text-overflow:ellipsis;">{{item.imageName}}</div>
				</div>
			</div>
			<div style="text-align: center;margin-top: 10px;">
				<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
	...
	export default {
		...
		methods: {
			...
			artiReleass() {
				...
				let photost = this.photos.toString();
				...
				if (this.commissionPrice == '') {
					...
					commissionPrices = this.commissionPrice
				}
				// if (this.buyReason == '') {
				// 	this.$notify({
				// 		title: '提示',
				...
				this.$http({
					...
					data: this.$http.adornData({
						...
						'commissionPrice': commissionPrices,
			...
			querenBtn(row) {
				...
				if (this.imgIndex == 5) {
					this.valueData2[0].skuImg = row.imageUrl
					this.centerDialogVisible = false
					this.centerDialogVisible2 = false
				}
				if (this.imgIndex == 6) {
