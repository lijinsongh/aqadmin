	...
	<el-tabs v-model="activeName" @tab-click="handleClick">
		<el-tab-pane label="提现管理" name="first">
			<div style="margin:2% 0;display: inline-block;">
				<span>提现类型：</span>
				<el-select v-model="statusType" placeholder="请选择提现状态" style="width:150px;" @change="selectStatus(statusType)">
					<el-option v-for="item in statusmaintype" :key="item.value" :label="item.label" :value="item.value">
					</el-option>
				</el-select>&nbsp;&nbsp;&nbsp;&nbsp;
			</div>
			<div style="margin:2% 0;display: inline-block;">
			...
			<div style="position: relative;display: inline-block;">
				<span>状态：</span>
				<el-select v-model="status" placeholder="请选择状态" style="width:150px;" @change="select(status)">
			...
			<div style="display: inline-block;">
				<el-button style='margin:0 0 20px 20px;' size="mini" type="primary" icon="document"
					@click="transferClcik()" :disabled="checkBoxData.length <= 0 || !isAuth('financeList:accounts')">
					批量转账
				</el-button>
			...
			<el-table @selection-change="changeFun" v-loading="tableDataLoading" :data="tableData">
				...
				<el-table-column fixed prop="relationId" label="会员/商户编号" width="120">
					<template slot-scope="scope">
						<span style="color: #4f9dec;cursor: pointer;" v-if="scope.row.relationId"
							@click="updates(scope.row)">{{scope.row.relationId}}</span>
						<span style="color: #4f9dec;cursor: pointer;" v-if="scope.row.storeId"
							@click="updatesShop(scope.row)">{{scope.row.storeId}}</span>	
					</template>
				</el-table-column>
				...
				<el-table-column prop="wxImg" label="微信提现二维码" width="100">
					<template slot-scope="scope">
						<img v-if="scope.row.wxImg" :src="scope.row.wxImg" style="width: 50px;height: 50px;"/>
					</template>
				</el-table-column>
				...
				<el-table-column fixed="right" label="操作" width="150">
					<template slot-scope="scope">
						<el-button size="mini" type="primary" :disabled="!isAuth('financeList:accounts')||scope.row.state != 0"
					...
					<div class="box">
						<div class="box_num">
							...
							<div class="text_color">
								<span>{{withdrawData.sumMoney ? withdrawData.sumMoney : 0}}</span>元
							</div>
						</div>
					</div>
				</el-col>
				<el-col :span="6" class="cards">
					<div class="box">
						<div class="box_num">
							...
							<div class="text_color">
								<span>{{withdrawData.stayMoney ? withdrawData.stayMoney :0}}</span>元
							</div>
						</div>
					</div>
				</el-col>
				<el-col :span="6" class="cards">
					<div class="box">
						<div class="box_num">
							...
							<div class="text_color">
								<span>{{withdrawData1.countNumberByTime ? withdrawData1.countNumberByTime : 0}}</span>笔
							</div>
						</div>
					</div>
				</el-col>
				<el-col :span="6" class="cards">
					<div class="box">
						<div class="box_num">
							...
							<div class="text_color">
								<span>{{withdrawData1.notOutMoneyByTime ? withdrawData1.notOutMoneyByTime :0}}</span>人
							</div>
						</div>
					</div>
				</el-col>
		...
		<el-tab-pane label="充值统计" name="four">
			<div>
				<el-select v-model="way" style="width:150px;margin-left: 10px;" @change="animeDat">
					<el-option v-for="item in ways" :key="item.value" :label="item.label" :value="item.value">
					</el-option>
			...
			<el-row>
				<el-col :span="6" class="cards">
					<div class="box">
						<div class="box_num">
							<div class="box_color">总充值金额</div>
							<div class="text_color">
								<span>{{chonzhiData.money ? chonzhiData.money : 0}}</span>元
							</div>
						</div>
					</div>
				</el-col>
				<el-col :span="6" class="cards">
					<div class="box">
						<div class="box_num">
							...
							<div class="text_color">
								<span>{{chonzhiData.weiXinMoney ? chonzhiData.weiXinMoney :0}}</span>元
							</div>
						</div>
					</div>
				</el-col>
				<el-col :span="6" class="cards">
					<div class="box">
						<div class="box_num">
							...
							<div class="text_color">
								<span>{{chonzhiData.zhiFuBaoMoney ? chonzhiData.zhiFuBaoMoney :0}}</span>元
							</div>
						</div>
					</div>
				</el-col>
		...
		<el-tab-pane label="充值记录" name="fourth">
			...
			<el-table v-loading="tableDataLoading" :data="rechargeData">
				...
				<el-table-column prop="userId" label="会员编号" width="100">
				</el-table-column>
	...
	export default {
		...
		methods: {
			...
			dataSelect() {
				this.tableDataLoading = true
				this.$http({
					url: this.$http.adornUrl2('/cash/selectCashList'),
					method: 'get',
					params: this.$http.adornParams({
						'page': this.page - 1,
						'size': this.size,
			...
			withdrawSelect() {
				this.$http({
					url: this.$http.adornUrl2('/cash/statisticsCashMoney'),
					method: 'get',
					params: this.$http.adornParams({
					...
					let returnData = data.data
					if(returnData.stayMoney){
						returnData.stayMoney = parseFloat(returnData.stayMoney).toFixed(2)
					}
					if(returnData.sumMoney){
						returnData.sumMoney = parseFloat(returnData.sumMoney).toFixed(2)
					}
				...
				this.$http({
					...
					let returnData = data.data
					this.withdrawData1 = returnData
					if(returnData.sumMoneyByTime){
			...
			chonzhiSelect() {
				this.$http({
					url: this.$http.adornUrl2('/statistics/payStatistics'),
					method: 'get',
					params: this.$http.adornParams({
						'time': this.info.stockDate,
					})
			...
			rechargeSelect() {
				...
				this.$http({
					url: this.$http.adornUrl2(
						`/cash/selectPayDetails?page=${this.page - 1}&size=${this.size}&startTime=${this.startTime}&endTime=${this.endTime}`
						),
					method: 'post',
					data: this.$http.adornData({})
				}).then(({
