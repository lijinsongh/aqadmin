  ...
  <nav class="site-navbar" :class="'site-navbar--' + navbarLayoutType">
    <div class="site-navbar__header">
      <h1 class="site-navbar__brand">
        <a class="site-navbar__brand-lg" href="javascript:;">省钱兄淘宝客+多商户管理系统</a>
      ...
      newsSelect() {
        ...
        this.$http({
		  ...
		  if (data.data.sum0 > 0) {
				_this.lodNums = 1
				_this.lodNum = 0
		    _this.chatNumMessage2 = data.data.sum0
		    console.log('订单提示',data.data)
		    this.$notify.info({
		      title: '消息',
		      dangerouslyUseHTMLString: true,
		      message: '有' + data.data.sum0 + '件商品提醒发货，请前往订单中心页面处理。',
		      offset: 80,
		      duration: 0,
		      type: "warning",
		      onClose() {
		  ...
		  if (data.data.sum1 > 0) {
				_this.lodNums = 1
				_this.lodNum = 0
		    _this.chatNumMessage2 = data.data.sum1
		    this.$notify.info({
		      title: '消息',
		      dangerouslyUseHTMLString: true,
		      message: '有' + data.data.sum1 + '条投诉，请前往店铺管理投诉管理处理。',
		      offset: 80,
		      duration: 0,
		      type: "warning",
		      onClose() {
		  ...
		  if (data.data.sum0 == 0&&data.data.sum1 == 0){
		    _this.chatNumMessage2 =0
				_this.lodNums = 0
				_this.lodNum = 0
		  }
