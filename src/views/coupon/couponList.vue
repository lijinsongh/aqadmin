		...
		<el-tabs v-model="activeName" @tab-click="handleClick">
			<el-tab-pane label="卡券列表" name="first">
				<div style="margin:2% 0;display: inline-block;">
					<span>卡券名称：</span>
					<el-input style="width: 150px;" @keydown.enter.native="select" clearable placeholder="请输入卡券名称"
						v-model="cardName"></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
					<span>卡号：</span>
					<el-input style="width: 150px;" @keydown.enter.native="select" clearable placeholder="请输入卡号"
						v-model="cardNo"></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
					<span>卡券类型：</span>
					<el-select v-model="cardType" placeholder="请选择类型" style="width:150px;"
						@change="selectTrigger(cardType)">
				...
				<div style="display: inline-block;">
					...
					<el-button style='margin-left:15px;' :disabled="!isAuth('couponList:add')" size="mini"
						type="primary" icon="document" @click="addNotice">生成卡券</el-button>
					<el-button style='margin-left:15px;' size="mini" type="success" icon="document" @click="derive">
						导出Excel</el-button>
					<el-button style='margin-left:15px;'
						:disabled="!isAuth('couponList:delete') || checkBoxData.length <= 0 " size="mini" type="danger"
				...
				<el-table @selection-change="changeFun" v-loading="tableDataLoading" :data="tableData.content"
					...
					<el-table-column prop="cardMoney" label="卡券金额(分)">
					...
					<el-table-column label="操作" width="120">
						<template slot-scope="scope">
							<el-button size="mini" type="danger" :disabled="!isAuth('couponList:delete')"
				...
				<div style="text-align: center;margin-top: 10px;">
					<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
						:page-sizes="[10, 15, 20, 25]" :page-size="size" :current-page="page"
				...
				<el-dialog title="生成卡券" :visible.sync="dialogFormVisible" center>
					...
					<div style="margin-bottom: 10px;">
						...
						<el-input style="width:50%;" v-model="cardNumber" min="0" type="number" placeholder="请输入卡券数量">
						</el-input>
					</div>
					<div style="margin-bottom: 10px;">
						<span style="width: 200px;display: inline-block;text-align: right;">卡券金额：</span>
						<el-input style="width:50%;" v-model="cardMoney" type="number" min="0"
							placeholder="请输入卡券金额  单位(元)"></el-input>
					</div>
					<div style="margin-bottom: 10px;">
			...
			<el-tab-pane label="卡券管理" name="second">
				...
				<el-table v-loading="tableDataLoading" :data="couponData.content" row-key="id">
					<el-table-column prop="id" label="编号" width="80">
					</el-table-column>
					<el-table-column prop="userId" label="用户id" width="150">
					</el-table-column>
					<el-table-column prop="userDeductQuota" label="用户抵扣总额度(元)">
						<template slot-scope="scope">
							<span>{{scope.row.userDeductQuota/100}}</span>
						</template>
					</el-table-column>
					<el-table-column prop="userFrozenAmount" label="用户冻结金额(元)">
						<template slot-scope="scope">
				...
				<div style="text-align: center;margin-top: 10px;">
					<el-pagination @size-change="handleSizeChange1" @current-change="handleCurrentChange1"
						:page-sizes="[10, 15, 20, 25]" :page-size="size" :current-page="page"
						layout="total,sizes, prev, pager, next,jumper" :total="couponData.totalElements">
					</el-pagination>
				</div>
			</el-tab-pane>
			<el-tab-pane label="抵扣明细" name="third">
				<div style="display: inline-block;float: right;margin: 2% 0;">
					...
					<el-button style='margin-left:15px;'
						:disabled="!isAuth('storeList:delete') || checkBoxData.length <= 0 " size="mini" type="danger"
						icon="document" @click="choideletes1()">批量删除</el-button>
				</div>
				<el-table @selection-change="changeFun" v-loading="tableDataLoading" :data="deductionData.content"
					row-key="id">
					<el-table-column type="selection">
					</el-table-column>
					<el-table-column prop="id" label="编号" width="80">
					...
					<el-table-column prop="couponAmount" label="金额">
						<template slot-scope="scope">
					...
					<el-table-column prop="productTitle" label="订单名称">
					</el-table-column>
					<el-table-column prop="createTime" label="创建时间">
					</el-table-column>
					<el-table-column label="操作" width="120">
				...
				<div style="text-align: center;margin-top: 10px;">
					<el-pagination @size-change="handleSizeChange2" @current-change="handleCurrentChange2"
						:page-sizes="[10, 15, 20, 25]" :page-size="size" :current-page="page"
						layout="total,sizes, prev, pager, next,jumper" :total="deductionData.totalElements">
					</el-pagination>
				</div>
			</el-tab-pane>
		</el-tabs>
			...
			deduction() {
				...
				this.$http({
					url: this.$http.adornUrl2(
						`/couponOrderDetail/findAllCouponOrderDetail?page=${page}&size=${this.size}`),
					method: 'post',
					data: this.$http.adornData({
						'userId': this.userId,
